...
# R/16, ng_poly_policy in R/18) must forward the shared mate-selection controls
...
attr(scores, "parent_K") <- K

n_crosses <- 8L
eps <- 1e-8

# --- 1. method = "evolution" is forwarded and runs ---
plan_evo <- ng_poly4x_ocs(scores, n_crosses = n_crosses, parent_K = K,
...
committed <- data.frame(parent1 = c("P01", "P17"), parent2 = c("P02", "P18"),
                        stringsAsFactors = FALSE)
plan_com <- ng_poly4x_ocs(scores, n_crosses = n_crosses, parent_K = K,
...
base <- ng_poly4x_ocs(scores, n_crosses = n_crosses, parent_K = K,
                      method = "greedy_local", lambda_group = 0)
base_gc <- attr(base, "summary")$group_coancestry
stopifnot(is.finite(base_gc))
tc <- ng_poly4x_ocs(scores, n_crosses = n_crosses, parent_K = K,
...
plan_gain <- ng_poly4x_ocs(scores, n_crosses = n_crosses, parent_K = K,
                           strategy = "high_gain", method = "greedy_local")
plan_div <- ng_poly4x_ocs(scores, n_crosses = n_crosses, parent_K = K,
...
plan_pol <- ng_poly4x_policy(scores, n_crosses = n_crosses, mode = "ocs",
                             parent_K = K, method = "greedy_local",
...
# --- 6. ng_poly_policy (subgenome, R/18) forwards controls via ... ---
sub_scores <- data.frame(parent1 = scores$parent1, parent2 = scores$parent2,
                         ...
                         stringsAsFactors = FALSE)
attr(sub_scores, "parent_K") <- K
plan_sub <- ng_poly_policy(sub_scores, n_crosses = n_crosses, mode = "ocs",
                           parent_K = K, method = "greedy_local",
                           committed_crosses = committed)
