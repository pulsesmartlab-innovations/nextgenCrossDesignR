# Any-ploidy polyploid mate design (R/46): ng_polyploid_score_crosses (analytic mean + kinship) and
# ng_polyploid_design_crosses (QC -> score -> native allocator with the full control suite).
...
for (ploidy in c(2L, 4L)) {
  ...
  # --- scorer contract ---
  scores <- ng_polyploid_score_crosses(dosage, effects, ploidy = ploidy)
  stopifnot(all(c("parent1", "parent2", "poly_mean", "pair_kinship") %in% names(scores)))
  stopifnot(nrow(scores) == choose(np, 2L))
  K <- attr(scores, "parent_kinship"); gebv <- attr(scores, "parent_gebv")
  ...
  plan <- ng_polyploid_design_crosses(dosage, n_crosses = 8L, ploidy = ploidy, effects = effects,
                                 method = "greedy_local")
  ...
  plan_ph <- ng_polyploid_design_crosses(dosage, n_crosses = 8L, ploidy = ploidy, phenotype = pheno,
                                    method = "greedy_local")
  ...
  plan_c <- ng_polyploid_design_crosses(dosage, n_crosses = 8L, ploidy = ploidy, effects = effects,
  ...
  plan_div <- ng_polyploid_design_crosses(dosage, n_crosses = 8L, ploidy = ploidy, effects = effects,
  ...
  plan_qc <- ng_polyploid_design_crosses(dosage, n_crosses = 8L, ploidy = ploidy, effects = effects,
  ...
  sc_add <- ng_polyploid_score_crosses(dosage, effects, ploidy = ploidy)
  stopifnot(all(c("poly_mean", "poly_var", "poly_usefulness") %in% names(sc_add)),
            all(sc_add$poly_var >= 0))
  plan_u <- ng_polyploid_design_crosses(dosage, n_crosses = 8L, ploidy = ploidy, effects = effects,
