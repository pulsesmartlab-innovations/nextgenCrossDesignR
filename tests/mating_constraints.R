...
scores$uc_dh_gebv <- rnorm(nrow(scores), 10, 3)
...
committed <- data.frame(parent1 = c("P01", "P29"), parent2 = c("P02", "P30"),
                        stringsAsFactors = FALSE)
plan_c <- ng_optimize_mating_plan(scores, n_crosses, parent_K = K,
...
plan_g <- ng_optimize_mating_plan(scores, n_crosses, parent_K = K,
                                  max_crosses_per_parent = 6L,
...
plan_q <- ng_optimize_mating_plan(scores, n_crosses, parent_K = K,
                                  max_crosses_per_parent = 6L,
