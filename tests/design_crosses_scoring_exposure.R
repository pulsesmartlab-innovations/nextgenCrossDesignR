...
# Both drive the recombination-aware within-family variance (dh_recomb_var); defaults must
...
stopifnot(isTRUE(all.equal(d0$scores$dh_recomb_var, d_dh$scores$dh_recomb_var)))
stopifnot(isTRUE(all.equal(d0$scores$dh_recomb_var, d_hal$scores$dh_recomb_var)))
# target and recomb_model reach the scorer and change the recombination variance
stopifnot(!isTRUE(all.equal(d_dh$scores$dh_recomb_var, d_ril$scores$dh_recomb_var)))
stopifnot(!isTRUE(all.equal(d_hal$scores$dh_recomb_var, d_kos$scores$dh_recomb_var)))
