...
wheat_decision <- ng_poly_model_select(
  crop = "wheat",
...
potato_decision <- ng_poly_model_select(
  crop = "potato",
...
sugarcane_decision <- ng_poly_model_select(
  crop = "sugarcane",
...
expect_error(
  ng_poly_model_select(crop = "wheat", inheritance_model = "disomic_subgenome"),
  "subgenome"
)
expect_error(
  ng_poly_model_select(crop = "wheat", inheritance_model = "disomic_subgenome", subgenome_names = c("A", "A")),
  "unique"
)
expect_error(
  ng_poly_model_select(crop = "wheat", inheritance_model = "disomic_subgenome", subgenome_names = c("A", "B"), ploidy_profile = c(A = 2L, B = 4L)),
  "diploid"
)
expect_error(
  ng_poly_model_select(crop = "wheat", inheritance_model = "disomic_subgenome", subgenome_names = c("A", "B"), ploidy_profile = c(A = 2.9, B = 2.1)),
  "integer"
)
expect_error(
  ng_poly_model_select(crop = "wheat", inheritance_model = "disomic_subgenome", subgenome_names = c("A", "B"), quad_prob = 0.1),
  "quadrivalent"
)
wheat_null_quad <- ng_poly_model_select(
  crop = "wheat",
  inheritance_model = "disomic_subgenome",
...
expect_error(
  ng_poly_model_select(crop = "wheat", inheritance_model = "unknown", subgenome_names = c("A", "B", "D")),
  "inheritance_model"
)
expect_error(
  ng_poly_model_select(crop = "potato", inheritance_model = "polysomic", ploidy = 4.9),
  "integer"
)
expect_error(
  ng_poly_model_select(crop = "mystery", inheritance_model = "unknown"),
  "inheritance_model"
)
...
checked_geno <- ng_poly_subgenome_as_dosage_list(geno_by_subgenome)
...
names(unnamed) <- NULL
expect_error(ng_poly_subgenome_as_dosage_list(unnamed), "named")

missing_subgenome_name <- geno_by_subgenome
names(missing_subgenome_name) <- c("A", NA_character_)
expect_error(ng_poly_subgenome_as_dosage_list(missing_subgenome_name), "named")

blank_subgenome_name <- geno_by_subgenome
names(blank_subgenome_name) <- c("A", "  ")
expect_error(ng_poly_subgenome_as_dosage_list(blank_subgenome_name), "named")

bad_dosage <- geno_by_subgenome
bad_dosage$A[1, 1] <- 3
expect_error(ng_poly_subgenome_as_dosage_list(bad_dosage), "0..2")

bad_rows <- geno_by_subgenome
rownames(bad_rows$B) <- c("P1", "P2", "PX")
expect_error(ng_poly_subgenome_as_dosage_list(bad_rows), "parent row names")

na_rows <- geno_by_subgenome
rownames(na_rows$A) <- c("P1", NA_character_, "P3")
expect_error(ng_poly_subgenome_as_dosage_list(na_rows), "parent row names")

blank_cols <- geno_by_subgenome
colnames(blank_cols$A) <- c("A_m1", " ")
expect_error(ng_poly_subgenome_as_dosage_list(blank_cols), "marker column names")

missing_cols <- geno_by_subgenome
colnames(missing_cols$A) <- NULL
expect_error(ng_poly_subgenome_as_dosage_list(missing_cols), "marker column names")

K <- ng_poly_subgenome_parent_relationship(geno_by_subgenome)
expected_K <- matrix(
  c(1, -1, 0,
    -1, 1, 0,
    0, 0, 0),
...
weighted_K <- ng_poly_subgenome_parent_relationship(geno_by_subgenome, weights = c(A = 2, B = 1))
...
weighted_distinct <- ng_poly_subgenome_parent_relationship(weighted_fixture, weights = c(A = 2, B = 1))
...
expect_error(ng_poly_subgenome_parent_relationship(geno_by_subgenome, weights = bad_weights), "weights")
...
scores <- ng_poly_subgenome_score_crosses(
...
stopifnot(!is.null(attr(scores, "parent_K")))

bad_effects <- effects_by_subgenome
bad_effects$A <- c(A_m1 = 0.5)
expect_error(
  ng_poly_subgenome_score_crosses(geno_by_subgenome, bad_effects, candidate_pairs = candidate_pairs),
  "marker"
)

gain_plan <- ng_poly_policy(scores, n_crosses = 2L, mode = "gain")
...
diversity_plan <- ng_poly_policy(scores, n_crosses = 2L, mode = "diversity")
...
ocs_plan <- ng_poly_policy(scores, n_crosses = 2L, mode = "ocs")
...
expect_error(ng_poly_policy(scores, n_crosses = 2L, mode = "invalid"), "mode")
