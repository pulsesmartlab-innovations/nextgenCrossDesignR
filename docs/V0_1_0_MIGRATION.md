...
| **T1.5** | `R/03_metrics.R`, `R/00_utils.R` | The DH/RIL variance formula assumes both parents are inbred (dosage ∈ {0, 2}). Heterozygous-parent input produced silently-wrong PMV. | New `ng_audit_inbred_dosage()` detects residual heterozygosity. `ng_score_crosses()` gains `assume_inbred = TRUE` (default) — errors out on outbred parents and points the user at `ng_exact_gms_additive_var()` (haplotype-based). Pass `assume_inbred = FALSE` to downgrade to a warning. |
