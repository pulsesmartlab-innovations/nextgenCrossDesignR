...
out_path <- env_chr(
  "NG_HEAD_TO_HEAD_VISUAL_REPORT_OUT",
  file.path(output_dir, paste0(prefix, "_visual_report.html"))
)

cat("Rendering head-to-head visual report\n")
cat("  prefix: ", prefix, "\n", sep = "")
cat("  output_dir: ", output_dir, "\n", sep = "")
cat("  out: ", out_path, "\n", sep = "")

written <- ng_write_head_to_head_visual_report(
  output_path = out_path,
  results_dir = output_dir,
  prefix = prefix
)

cat("Wrote head-to-head visual report: ", written, "\n", sep = "")
