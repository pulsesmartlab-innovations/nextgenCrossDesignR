  ...
  list(
    ...
    enum("multi_trait_method", "Multi-trait method", "objective", "auto",
      c(auto = "Auto (equal weights from directions)", weighted = "Weighted index",
        ...
        threshold = "Threshold (soft by default)",
        rank_sum = "Rank summation index (Mulamba-Mock, weighted)"),
      capability = "multitrait_auto"),
    ...
    num("min_cv_predictive_r2", "Minimum marker prediction quality (out-of-fold R2)",
        ...
        capability = "dh_ril_pmv_scoring",
        note = paste0("The acceptability bar for the marker model, and the one control ",
                      "here a breeding programme should set for itself. ",
                      "READ THE SCALE: this is an R2, not a correlation. The 0.35 default ",
                      "is about r = 0.59 on the correlation scale breeders usually quote, ",
                      "which is a demanding bar rather than a modest one; 0.20 is about ",
                      "r = 0.45. It is also measured against PHENOTYPE, not breeding ",
                      "value, so heritability attenuates it and it UNDERSTATES accuracy ",
                      "against breeding value -- with h2 = 0.3, an R2 of 0.20 can mean ",
                      "markers that predict breeding value well. ",
                      "WHAT IT GOVERNS: cross MEANS only. At or above the bar they are ",
                      "GEBVs; below it they fall back to the phenotypic mid-parent while ",
                      "the within-family variance stays marker-derived in every tier, so ",
                      "a below-bar trait yields a phenotypic mean beside a marker-based ",
                      "spread. Lowering it is not a way to accept anything: an ",
                      "unevaluable (fewer than 10 records) or negative R2 refuses the run ",
                      "regardless, because that is not a low standard but no standard. ",
                      "One threshold applies to every trait in the run, though ",
                      "acceptability genuinely differs by trait with heritability and ",
                      "phenotyping effort. This is the ONLY control deciding whether a ",
                      "trait's marker effects are good enough to use; it replaced the ",
                      "retired min_effect_reliability, which gated a PEV-based ",
                      "reliability this package never computes and which answers a ",
                      "question about unphenotyped selection candidates rather than ",
                      "about the phenotyped parents being crossed.")),
    enum("effect_gate", "Marker-effect reliability gate", "scoring", "on",
      c(on  = "On (refuse a run the markers cannot support)",
        off = "Off - proceed without the gate, on markers the engine would refuse"),
      capability = "dh_ril_pmv_scoring"),
