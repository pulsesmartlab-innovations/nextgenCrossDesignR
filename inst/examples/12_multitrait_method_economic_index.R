# Multi-trait method example 3: economic_index.
#
# Use economic_index when traits have economic or breeding-program values.
# Put those values in the direction file as economic_weight.
...
set.seed(20260623)
out_dir <- file.path(tempdir(), "nextgenCrossDesign_multitrait_economic_index")
...
# Parameters specific to multi_trait_method = "economic_index".
multi_trait_method <- "economic_index"
trait_weights <- NULL
threshold_policy <- "soft"

parameter_notes <- data.frame(
  parameter = c("multi_trait_method", "economic_weight", "trait_weights", "threshold_policy"),
  value = c(multi_trait_method,
            paste(trait_direction$Trait, trait_direction$economic_weight, sep = "=", collapse = ", "),
            "NULL", threshold_policy),
  when_to_change = c(
    "Use economic_index when trait economic or breeding values are supplied.",
    "Set in the direction file; larger values mean stronger economic importance.",
    "Keep NULL when economic_weight is supplied in the direction file.",
...
economic_index_coefficients <- attr(result$candidate_crosses, "multi_trait")$economic_index_coefficients

stopifnot(inherits(result, "ng_cross_prediction_result"))
stopifnot(identical(result$objective$method, "economic_index"))
stopifnot(identical(result$objective$diagnostics$method_reason, "requested_economic_index"))
stopifnot(!is.null(economic_index_coefficients))
stopifnot(all(is.finite(economic_index_coefficients)))

parameter_notes
economic_index_coefficients
head(result$selected_crosses)

message("Multi-trait economic_index example completed.")
